...
public class AppDAO implements AppDAOInterface{
    ...
    public EstateModel getEstateById(String id) {
        ...
        if(count < 1){
            ...
            return new EstateModel(
                    ...
                    cursor.getInt(cursor.getColumnIndex(EstateTblDescription.KEY_ID.label)),
            ...
            while(cursor.moveToNext()) {
                try {
                    ret.add(new TenantModel(
                            ...
                            cursor.getInt(cursor.getColumnIndex(TenantTblDescription.KEY_ID.label)),
                            cursor.getInt(cursor.getColumnIndex(TenantTblDescription.KEY_ESTATE_ID.label)),
    ...
    public TenantModel addTenant(TenantModel newtenant, EstateModel property) {
        ...
        content.put(TenantTblDescription.KEY_ID.label, newtenant.id);
        content.put(TenantTblDescription.KEY_NAME.label, newtenant.name);
        content.put(TenantTblDescription.KEY_ESTATE_ID.label, property.id);
        content.put(TenantTblDescription.CONTACTS.label, newtenant.contacts);
        content.put(TenantTblDescription.BALANCE.label, newtenant.balance.intValue());
        content.put(TenantTblDescription.RENT.label, newtenant.rent.intValue());
    ...
    public boolean deleteEstate(EstateModel property) {
        int doneDelete = 0;
        database = mDbHelper.getWritableDatabase();
        doneDelete = database.delete(ESTATE_TABLE, " "+EstateTblDescription.KEY_ID.label+" = ? ", new String[]{Integer.toString(property.id)});
    ...
    public boolean deleteTenant(TenantModel tenant, EstateModel property) {
        int doneDelete = 0;
        database = mDbHelper.getWritableDatabase();
        doneDelete = database.delete(TENANT_TABLE, " "+EstateTblDescription.KEY_ID.label+" = ? AND "+TenantTblDescription.KEY_ID.label+" = ? ", new String[]{Integer.toString(property.id), Integer.toString(tenant.id)});
        Log.w(TAG, Integer.toString(doneDelete));
        database.close();
        return doneDelete > 0;
    }
    ...
    public EstateModel updateEstate(EstateModel property) {
        ...
        content.put(EstateTblDescription.KEY_ID.label, property.id);
        ...
        int rowsaffected = database.update(TENANT_TABLE, content, " "+EstateTblDescription.KEY_ID.label+" = ? ", new String[]{Integer.toString(property.id)});
    ...
    public TenantModel updateTenant(TenantModel newtenant, EstateModel estate) {
        ...
        if(estate!=null)
            content.put(TenantTblDescription.KEY_ESTATE_ID.label, estate.id);
        else
            content.put(TenantTblDescription.KEY_ESTATE_ID.label, newtenant.estateId);
        ...
        long rowId = database.update(TENANT_TABLE,  content, " "+TenantTblDescription.KEY_ID.label+" = ? ", new String[]{Integer.toString(newtenant.id)} );
        database.close();
    ...
    private static final String ESTATE_TABLE_CREATE = "CREATE TABLE IF NOT EXISTS "
            + ESTATE_TABLE + " (" +
            EstateTblDescription.KEY_ID.label + " "+EstateTblDescription.KEY_ID.type+" PRIMARY KEY AUTOINCREMENT," +
    ...
    private static final String TENANT_TABLE_CREATE = "CREATE TABLE IF NOT EXISTS "
            + TENANT_TABLE + " (" +
            //TenantTblDescription.KEY_ID.label + " "+TenantTblDescription.KEY_ID.type+" PRIMARY KEY AUTOINCREMENT," +
            TenantTblDescription.KEY_ID.label + " "+TenantTblDescription.KEY_ID.type+" ," +
    ...
    public void uploadData(){
        ...
        addEstate(new EstateModel( 1,"KN Apartment","Kiambu Rd",EstateModel.TYPE.apartment));

        EstateModel offices = new EstateModel( 2,"KN Plaza","Kiambu Rd",EstateModel.TYPE.commercial);
        addEstate(offices);

        try {
            addTenant(new TenantModel(1,"Mtu Moja", "N1", "Lounge hall", dateFormat.parse("2018/11/01"), TenantModel.STATUS.paid, BigDecimal.valueOf(10000.00), BigDecimal.valueOf(0.00), "0708555121"), offices);
            addTenant(new TenantModel(2,"Mtu Mwingine", "N3", "Office 1", dateFormat.parse("2018/11/01"), TenantModel.STATUS.paid, BigDecimal.valueOf(3000), BigDecimal.valueOf(0), "0708555222"), offices);
            addTenant(new TenantModel(3,"Watu Wengi", "N4", "Office 2", dateFormat.parse("2018/11/01"), TenantModel.STATUS.paid, BigDecimal.valueOf(1000), BigDecimal.valueOf(1000), "0708555444"), offices);
        }catch(Exception e){
